...
def def_process_user(ti):
    ...
    processed_user.to_csv('/tmp/output_user.csv', index=None,header=False)
                          

def def_store_user():
    hook = PostgresHook(postgres_conn_id='postgres')
    hook.copy_expert(
        sql="COPY users_final from stdin WITH DELIMITER as ','",
...
with DAG(
    ...
    target_store_user = PythonOperator(
        task_id = 'target_store_user',
        python_callable=def_store_user

    )
    create_table >> api_available >> extract_user >> process_user >> target_store_user